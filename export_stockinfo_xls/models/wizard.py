...
class StockReport(models.TransientModel):
    ...
    route_ids = fields.Many2many('stock.location.route',string="API")
    # location_id = fields.Many2one('stock.location', string='Locations', required=True)
    ...
    def get_lines(self,**args):
        lines = []
        route_name = args['route_ids'].mapped('id')
        categ_id = args['d'].mapped('id')
        # if location:
        #     cate_products = self.env['product.product'].search([('categ_id', 'in', categ_id), ('type', '=', 'product'),
        #                                                         ('location_id', '=', location)])
        if args['route_ids'] :
            categ_products = self.env['product.product'].search(['&','&','|',('categ_id', 'in', categ_id),('create_date', '>',args['start_date']),('create_date', '<=',args['end_date']),('route_ids','in',route_name)])
        elif args['start_date'] or args['end_date']:
            categ_products = self.env['product.product'].search(['&','|',('categ_id', 'in', categ_id),('create_date', '>',args['start_date']),('create_date', '<=',args['end_date'])])
        elif categ_id:
        ...
        params = args['i'], product_ids if product_ids else (0, 0)
        ...
        for obj in categ_products:
            ...
            opening = self.env['stock.valuation.layer'].search([('product_id','=',obj.id),('create_date','<',args['start_date'])],limit=1)
            # balance=0.0
            # for open_value in opening:
            #     balance+=open_value.quantity
            stock_increase = self.env['stock.valuation.layer'].search([('product_id','=',obj.id),('create_date','>=',args['start_date']),('create_date','<=',args['end_date']),('quantity','>',0)],limit=1)
            # total=0.0
            # for stock_value in stock_increase:
            #     total+=stock_value.quantity
            stock_decrease = self.env['stock.valuation.layer'].search([('product_id','=',obj.id),('create_date','>=',args['start_date']),('create_date','<=',args['end_date']),('quantity','<',0)],limit=1)
            # total_val1=0.0
            # for stock_dec in stock_decrease:
            #     total_val1 += stock_dec.quantity
            route_id = self.env['stock.location.route'].search([('id','in',route_name)],limit=1)
            closing = opening.quantity+stock_increase.quantity+stock_decrease.quantity
            virtual_available = obj.with_context({'warehouse': args['i']}).virtual_available
            outgoing_qty = obj.with_context({'warehouse': args['i']}).outgoing_qty
            incoming_qty = obj.with_context({'warehouse': args['i']}).incoming_qty
            ...
            vals = {
                ...
                'net_on_hand': obj.with_context({'warehouse': args['i']}).qty_available,
                'total_value': value,
    ...
    def get_xlsx_report(self, data, response):
        ...
        print(lines)
        # location = lines.location_id.display_name
        start_date = lines.start_date
        print('start date',start_date)
        end_date = lines.end_date
        print('end date',end_date)
        for route in lines.route_ids:
            route_ids= route
        ...
        p_col_no1 = 7
        for i in get_warehouse[0]:
            sheet.merge_range(9, p_col_no1,9,p_col_no1+1,'Opening Balance', format21)
            sheet.write(9, p_col_no1+1, 'Available', format21)
            sheet.write(9, p_col_no1 + 2, 'Route', format21)
            sheet.write(9, p_col_no1 + 3, 'Incoming', format21)
            sheet.write(9, p_col_no1 + 4, 'Outgoing', format21)
            sheet.merge_range(9, p_col_no1 + 5, 9, p_col_no1 + 6, 'Net On Hand', format21)
            sheet.merge_range(9, p_col_no1 + 7, 9, p_col_no1 + 8, 'Total Sold', format21)
            sheet.merge_range(9, p_col_no1 + 9, 9, p_col_no1 + 10, 'Total Purchased', format21)
            sheet.write(9, p_col_no1 + 11, 'Valuation', format21)
            sheet.merge_range(9, p_col_no1 + 12,9,p_col_no1+13,'Closing Balance', format21)
            p_col_no1 = p_col_no1 + 14
        prod_row = 10
        prod_col = 0
        for i in get_warehouse[1]:
            get_line = self.get_lines(d=d,i=i,start_date=start_date,end_date=end_date,route_ids=route_ids)
            for each in get_line:
                ...
                if each['opening'] < 0:
                    sheet.merge_range(prod_row, prod_col + 7,prod_row,prod_col+8, each['opening'], red_mark)
                else:
                    sheet.merge_range(prod_row, prod_col + 7,prod_row,prod_col+8, each['opening'], font_size_8)
                prod_row = prod_row + 1
            break
        prod_row = 10
        prod_col = 8
        for i in get_warehouse[1]:
            ...
            for each in get_line:
                ...
                if each['route']:
                    sheet.write(prod_row, prod_col + 1, each['route'], font_size_8)
                else:
                    sheet.write(prod_row, prod_col + 1,'', font_size_8)
                if each['incoming'] < 0:
                ...
                if each['closing'] < 0:
                    sheet.merge_range(prod_row, prod_col + 11, prod_row, prod_col + 12,['closing'], red_mark)
                else:
                    sheet.merge_range(prod_row, prod_col + 11,prod_row, prod_col + 12, each['closing'], font_size_8)
                prod_row = prod_row + 1
            prod_row = 10
            prod_col = prod_col + 13
        workbook.close()
        output.seek(0)
